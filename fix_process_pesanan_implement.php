<?php
// File implementasi untuk memperbaiki process_pesanan.php
// Jalankan file ini untuk menerapkan perubahan langsung ke process_pesanan.php

// Pastikan kita memiliki akses ke file
if (!file_exists('process_pesanan.php')) {
    die("Error: File process_pesanan.php tidak ditemukan!");
}

// Baca isi file process_pesanan.php
$content = file_get_contents('process_pesanan.php');
if ($content === false) {
    die("Error: Gagal membaca file process_pesanan.php!");
}

// Backup file asli terlebih dahulu
$backup_file = 'process_pesanan.php.bak_' . date('Ymd_His');
if (!file_put_contents($backup_file, $content)) {
    die("Error: Gagal membuat backup file!");
}

// Perbaikan: Ganti seluruh isi file dengan versi yang diperbaiki
$new_content = '<?php
...
try {
    ...
    if ($action == \'process\') {
        ...
        // Update pesanan status to processed
        $update_query = "UPDATE pesanan_barang SET status = \'processed\' WHERE id_pesanan = ?";
        $update_stmt = mysqli_prepare($conn, $update_query);
        mysqli_stmt_bind_param($update_stmt, "i", $id_pesanan);
        
        if (!mysqli_stmt_execute($update_stmt)) {
            throw new Exception("Gagal memperbarui status pesanan: " . mysqli_stmt_error($update_stmt));
        }
    ...
    elseif ($action == \'cancel\') {
        ...
        $update_query = "UPDATE pesanan_barang SET status = \'canceled\' WHERE id_pesanan = ?";
        $update_stmt = mysqli_prepare($conn, $update_query);
        mysqli_stmt_bind_param($update_stmt, "i", $id_pesanan);
...
if (!file_put_contents('process_pesanan.php', $new_content)) {
    die("Error: Gagal menulis ke file process_pesanan.php!");
}

echo "Perbaikan berhasil diterapkan ke file process_pesanan.php!<br>";
