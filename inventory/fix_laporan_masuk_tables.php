...
<html>
<head>
    <title>Fix Laporan Masuk Tables</title>
    <style>
        body {
...
<body>
    <div class="container">
        <h1>Fix Laporan Masuk Tables</h1>
        ...
        try {
            // Step 1: Check if laporan_masuk_detail exists with old structure
            $check_table = $conn->query("DESCRIBE laporan_masuk_detail");
            
            if ($check_table) {
                // Table exists, check its structure
                $has_old_structure = false;
                while ($row = $check_table->fetch_assoc()) {
                    if ($row['Field'] == 'id_laporan_masuk') {
                ...
                if ($has_old_structure) {
                    ...
                    echo "<h3>Backing up existing data from laporan_masuk_detail</h3>";
                    $backup_sql = "CREATE TEMPORARY TABLE tmp_laporan_masuk_detail 
                                  SELECT * FROM laporan_masuk_detail";
                    executeSQL($conn, $backup_sql, "Create temporary backup table");
                    
                    // Drop the old table
                    echo "<h3>Dropping old table structure</h3>";
                    $drop_sql = "DROP TABLE laporan_masuk_detail";
                    executeSQL($conn, $drop_sql, "Drop old table");
                }
            }
            ...
            // Step 3: Restore data from backup if it existed
            if (isset($has_old_structure) && $has_old_structure) {
                echo "<h3>Restoring data from backup</h3>";
                $restore_sql = "INSERT INTO laporan_masuk_detail (id_laporan, id_masuk)
                              SELECT id_laporan_masuk, id_masuk FROM tmp_laporan_masuk_detail";
                executeSQL($conn, $restore_sql, "Restore data from backup");
            }
            
            // Step 4: Insert missing records for existing laporan_masuk entries
