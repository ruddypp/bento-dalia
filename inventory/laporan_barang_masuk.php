...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['simpan_laporan'])) {
    ...
    if (empty($errors)) {
        ...
        try {
            ...
            if ($hasIdPengguna) {
                $masuk_query = "INSERT INTO barang_masuk (id_barang, tanggal_masuk, id_pengguna, qty_masuk, id_supplier) VALUES (?, ?, ?, ?, ?)";
                $masuk_stmt = $conn->prepare($masuk_query);
                if (!$masuk_stmt) {
                    throw new Exception("Error preparing barang_masuk insert query: " . $conn->error);
                }
                $masuk_stmt->bind_param("isidi", $barang_id, $tanggal_masuk, $user_id, $jumlah, $supplier_id);
            } else {
