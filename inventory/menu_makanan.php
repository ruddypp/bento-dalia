...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_menu'])) {
    ...
    if (empty($errors)) {
        try {
            ...
            if (!empty($id_menu)) {
                // Mode edit
                $query = "UPDATE menu SET nama_menu = ?, kategori = ?, harga = ?, bahan = ?, deskripsi = ?";
                ...
                if (!empty($foto)) {
                    ...
                    $stmt->bind_param("ssdssi", $nama_menu, $kategori, $harga, $bahan, $deskripsi, $foto, $id_menu);
                } else {
                    $query .= " WHERE id_menu = ?";
                    $stmt = $conn->prepare($query);
                    $stmt->bind_param("ssdsi", $nama_menu, $kategori, $harga, $bahan, $deskripsi, $id_menu);
                }
                ...
                // Mode tambah
                $query = "INSERT INTO menu (nama_menu, kategori, harga, bahan, deskripsi, foto) VALUES (?, ?, ?, ?, ?, ?)";
                $stmt = $conn->prepare($query);
                $stmt->bind_param("ssdsss", $nama_menu, $kategori, $harga, $bahan, $deskripsi, $foto);
        ...
        <form method="POST" action="<?= $_SERVER['PHP_SELF'] ?>" enctype="multipart/form-data" class="transition-all duration-300">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="form-group transition-all duration-200">
                    ...
                    <textarea id="bahan" name="bahan" rows="3" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-200" required><?= htmlspecialchars($bahan) ?></textarea>
                    <p class="text-xs text-gray-500 mt-1">Pisahkan dengan koma, contoh: Tepung, Telur, Gula</p>
                </div>
            ...
            <div class="md:w-1/2">
                <div class="mb-6">
                    ...
                    <div class="bg-green-50 p-4 rounded-lg">
                        <div class="mb-4">
                            <h5 class="text-sm font-semibold text-gray-700 mb-1">Harga:</h5>
                            <p class="text-xl font-bold text-green-600">${menuPrice}</p>
                        </div>
                        <div class="mb-4">
