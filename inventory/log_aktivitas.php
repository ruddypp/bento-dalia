...
if ($user_filter > 0) {
    $where_clause .= " WHERE l.id_pengguna = ?";
    $params[] = $user_filter;
    $types .= "i";
}
...
// Get logs data
$query = "SELECT l.*, p.nama_pengguna, p.username 
          FROM log_aktivitas l 
          JOIN pengguna p ON l.id_pengguna = p.id_pengguna" . $where_clause . "
...
$users_query = "SELECT id_pengguna, nama_pengguna, username FROM pengguna ORDER BY nama_pengguna ASC";
        ...
        <form method="GET" action="" class="flex flex-wrap gap-4 items-end">
            <div>
                ...
                <select id="user_id" name="user_id" class="shadow-sm border border-gray-300 rounded py-2 px-3 text-gray-700 text-sm leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    ...
                    <option value="<?= $user['id_pengguna'] ?>" <?= ($user_filter == $user['id_pengguna']) ? 'selected' : '' ?>>
            ...
            <tbody class="divide-y divide-gray-100">
                <?php if (mysqli_num_rows($logs) > 0): ?>
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="py-2 px-3 text-sm font-medium"><?= $log['nama_pengguna'] ?> <span class="text-xs text-gray-500">(<?= $log['username'] ?>)</span></td>
