...
if (isset($_POST['update_profile'])) {
    ...
    $query = "UPDATE pengguna SET nama_pengguna = ?, email = ? WHERE id_pengguna = ?";
        ...
        <div class="md:w-1/3 bg-gradient-to-br from-blue-600 to-blue-800 p-8 text-white">
            <div class="flex flex-col items-center justify-center h-full">
                <div class="w-32 h-32 rounded-full bg-white/20 flex items-center justify-center border-4 border-white/30 mb-6">
                    <span class="text-5xl font-bold"><?= strtoupper(substr($user['nama_pengguna'], 0, 1)) ?></span>
                </div>
                <h2 class="text-2xl font-bold text-center mb-2"><?= $user['nama_pengguna'] ?></h2>
                <p class="bg-blue-500/40 px-4 py-1 rounded-full text-sm text-center"><?= $user['nama_aktor'] ?></p>
                ...
                <div class="w-full mt-8">
                    ...
                    <div class="flex items-center mb-4">
                        <i class="fas fa-envelope w-6 text-blue-300"></i>
                        <span class="ml-2"><?= $user['email'] ?></span>
                    </div>
                    <div class="flex items-center">
                        <i class="fas fa-shield-alt w-6 text-blue-300"></i>
                        <span class="ml-2">ID: <?= $user['id_pengguna'] ?></span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="md:w-2/3 p-6">
            ...
            <div class="tab-content mt-6">
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="text" id="role" 
                                        class="pl-10 pr-3 py-2 border border-gray-300 rounded-lg w-full bg-gray-100 text-gray-600 cursor-not-allowed" 
                                        value="<?= $user['nama_aktor'] ?>" disabled readonly>
                                </div>
                            </div>
                        </div>
                ...
                <div id="activity-tab" class="tab-pane hidden">
                    ...
                    $query = "SELECT * FROM log_aktivitas WHERE id_pengguna = ? ORDER BY waktu DESC LIMIT 10";
                    $stmt = mysqli_prepare($conn, $query);
                    mysqli_stmt_bind_param($stmt, "i", $user_id);
                    mysqli_stmt_execute($stmt);
                    $result = mysqli_stmt_get_result($stmt);
