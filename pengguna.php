...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['add_user'])) {
        ...
        $alamat_user = sanitize($_POST['alamat_user']);
        $no_tlp_user = (int)$_POST['no_tlp_user'];
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            // Insert user
            $query = "INSERT INTO users (nama_lengkap, username, password, jenis_kelamin, alamat_user, no_tlp_user) VALUES (?, ?, ?, ?, ?, ?)";
            $stmt = mysqli_prepare($conn, $query);
            mysqli_stmt_bind_param($stmt, "sssssi", $nama, $username, $password, $jenis_kelamin, $alamat_user, $no_tlp_user);
    ...
    if (isset($_POST['edit_user'])) {
        ...
        if (!empty($password)) {
            ...
            $query = "UPDATE users SET nama_lengkap = ?, jenis_kelamin = ?, alamat_user = ?, no_tlp_user = ?, password = ? WHERE id_user = ?";
            $stmt = mysqli_prepare($conn, $query);
            mysqli_stmt_bind_param($stmt, "sssisi", $nama, $jenis_kelamin, $alamat_user, $no_tlp_user, $password, $id_user);
        } else {
            // Update tanpa mengubah password
            $query = "UPDATE users SET nama_lengkap = ?, jenis_kelamin = ?, alamat_user = ?, no_tlp_user = ? WHERE id_user = ?";
            $stmt = mysqli_prepare($conn, $query);
            mysqli_stmt_bind_param($stmt, "sssii", $nama, $jenis_kelamin, $alamat_user, $no_tlp_user, $id_user);
        }
        
        if (mysqli_stmt_execute($stmt)) {
            logActivity($_SESSION['user_id'], "Mengubah data pengguna ID: $id_user");
    ...
    if (isset($_POST['delete_user'])) {
        ...
        $query = "SELECT COUNT(*) as total FROM log_aktivitas WHERE id_user = ?";
        $stmt = mysqli_prepare($conn, $query);
        ...
        if ($row['total'] > 0) {
            ...
            $query = "DELETE FROM user_roles WHERE id_user = ?";
            $stmt = mysqli_prepare($conn, $query);
            mysqli_stmt_bind_param($stmt, "i", $id_user);
            mysqli_stmt_execute($stmt);
            ...
            if (mysqli_stmt_execute($stmt)) {
                logActivity($_SESSION['user_id'], "Menghapus pengguna ID: $id_user");
...
$query = "SELECT id_role, nama_role FROM roles ORDER BY nama_role ASC";
        ...
        <table class="min-w-full bg-white data-table">
            <thead>
                <tr class="border-b border-gray-200">
                    ...
                    <th class="py-2 px-3 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">Jenis Kelamin</th>
                    <th class="py-2 px-3 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">Tanggal Daftar</th>
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50">
                    <td class="py-2 px-3 text-sm"><?= $user['id_user'] ?></td>
                    <td class="py-2 px-3 text-sm font-medium"><?= $user['nama_lengkap'] ?></td>
                    <td class="py-2 px-3 text-sm text-gray-600"><?= $user['username'] ?></td>
                    <td class="py-2 px-3 text-sm">
                        <span class="px-2 py-0.5 <?= ($user['jenis_kelamin'] == 'Laki - Laki') ? 'bg-blue-100 text-blue-800' : 'bg-pink-100 text-pink-800' ?> rounded-full text-xs">
                            <?= $user['jenis_kelamin'] ?: 'Tidak diketahui' ?>
                        </span>
                    </td>
                    <td class="py-2 px-3 text-sm text-gray-600"><?= date('d/m/Y', strtotime($user['tanggal_daftar'])) ?></td>
                    <td class="py-2 px-3 text-sm">
                        <button class="text-blue-500 hover:text-blue-700 mr-2" 
                                onclick="editUser(<?= $user['id_user'] ?>, '<?= $user['nama_lengkap'] ?>', '<?= $user['username'] ?>')">
                            <i class="fas fa-edit"></i>
                        </button>
                        <?php if ($user['id_user'] != $_SESSION['user_id']): ?>
                        <button class="text-red-500 hover:text-red-700" 
                                onclick="deleteUser(<?= $user['id_user'] ?>, '<?= $user['nama_lengkap'] ?>')">
        ...
        <div class="mt-3">
            ...
            <form id="addUserForm" method="POST" action="" class="mt-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="mb-4">
                        <label class="block text-gray-700 text-sm font-semibold mb-2" for="alamat_user">
                            Alamat
                        </label>
                    ...
                    <div class="mb-4">
                        <label class="block text-gray-700 text-sm font-semibold mb-2" for="no_tlp_user">
                            No. Telepon
                        </label>
                        <input type="number" id="no_tlp_user" name="no_tlp_user" 
                               class="shadow-sm border border-gray-300 rounded w-full py-2 px-3 text-gray-700 text-sm leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
    ...
    <div class="relative top-10 mx-auto p-5 border w-11/12 md:w-2/3 lg:w-1/2 shadow-lg rounded-md bg-white">
        <div class="mt-3">
            ...
            <form id="editUserForm" method="POST" action="" class="mt-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="mb-4">
                        <label class="block text-gray-700 text-sm font-semibold mb-2" for="edit_nama_lengkap">
                            Nama Lengkap
                        </label>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" id="edit_username" name="username" required 
                               class="shadow-sm border border-gray-300 rounded w-full py-2 px-3 text-gray-700 text-sm leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" readonly>
                    </div>
                    ...
                    <div class="mb-4">
                        <label class="block text-gray-700 text-sm font-semibold mb-2" for="edit_jenis_kelamin">
                            Jenis Kelamin
                        </label>
                    ...
                    <div class="mb-4">
                        <label class="block text-gray-700 text-sm font-semibold mb-2" for="edit_alamat_user">
                            Alamat
                        </label>
                        <input type="text" id="edit_alamat_user" name="alamat_user" 
                               class="shadow-sm border border-gray-300 rounded w-full py-2 px-3 text-gray-700 text-sm leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    </div>
                    
                    <div class="mb-4">
                        <label class="block text-gray-700 text-sm font-semibold mb-2" for="edit_no_tlp_user">
                            No. Telepon
                        </label>
                        <input type="number" id="edit_no_tlp_user" name="no_tlp_user" 
                               class="shadow-sm border border-gray-300 rounded w-full py-2 px-3 text-gray-700 text-sm leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    </div>
                </div>
